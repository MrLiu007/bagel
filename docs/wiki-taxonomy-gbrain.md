...
全类型资源（新闻 / GitHub / 论文 / 教育 / 模型 / 股票 / 自媒体…）在同一用户空间统一挂到 taxonomy 主题上，GBrain 一张图呈现。

## 操作

- **手动**：采集页 →「编译 Wiki」→ 任务 kind `compile_wiki`
- **定时**：设置 → 定时任务 →「定时编译 Wiki」（默认每日 04:10，`enable_wiki_compile`）
- **浏览**：`/briefs/space` GBrain（主题实线 about / 先修虚线 prerequisite）
...
- `/briefs/space?view=board`：搜索统计、关键词、类型与数据源（原看板）
